...
public class GetList {
    ...
    @SuppressWarnings("unchecked")
    public static List<AssignedJobOrder> AssignedJobOrders(Machine m) {
        Session session = HibernateUtil.getSessionFactory().openSession();
        Query q = session.createQuery("from AssignedJobOrder where Machine = :machine").setParameter("machine", m);
        List<AssignedJobOrder> ret = q.list();
        session.close();
        return ret;
    }
